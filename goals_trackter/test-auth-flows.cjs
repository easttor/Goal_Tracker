...
async function testAuthFlows() {
  ...
  if (signUpError) {
    console.log('X Sign up error:', signUpError.message);
  } else {
  ...
  if (signInError && signInError.message.includes('Invalid login credentials')) {
    ...
    if (demoSignUpError) {
      console.log('X Demo account creation error:', demoSignUpError.message);
    } else {
      ...
      if (retrySignInError) {
        console.log('X Demo sign in error after creation:', retrySignInError.message);
      } else {
        console.log('+ Demo sign in successful after creation');
      }
    }
  } else if (signInError) {
    console.log('X Demo sign in error:', signInError.message);
  } else {
  ...
  if (newSignInError) {
    console.log('X New account sign in error:', newSignInError.message);
  } else {
  ...
  if (sessionError) {
    console.log('X Session error:', sessionError.message);
  } else if (sessionData.session) {
    console.log('+ Session exists');
    console.log('  User:', sessionData.session.user?.email);
  } else {
    console.log('X No active session');
  }
