...
-- Created: 2025-11-04
-- Description: Adds social networking capabilities including
--              user following, goal sharing, teams, comments,
--              and activity feeds
-- =====================================================
...
-- Add team_goal_id column if comments table already exists
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'comments') THEN
        ALTER TABLE public.comments ADD COLUMN IF NOT EXISTS team_goal_id UUID REFERENCES public.team_goals(id) ON DELETE CASCADE;
        ALTER TABLE public.comments ADD COLUMN IF NOT EXISTS mentions UUID[];
    END IF;
END $$;

-- Add check constraint for comments
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'comments_entity_check' 
...
CREATE INDEX IF NOT EXISTS idx_comments_goal ON public.comments(goal_id) WHERE goal_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_comments_task ON public.comments(task_id) WHERE task_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_comments_team_goal ON public.comments(team_goal_id) WHERE team_goal_id IS NOT NULL;
...
DROP POLICY IF EXISTS "Users can view shares for their goals" ON public.goal_shares;
...
DROP POLICY IF EXISTS "Goal owners can create shares" ON public.goal_shares;
CREATE POLICY "Goal owners can create shares"
    ON public.goal_shares FOR INSERT
    WITH CHECK (owner_id = auth.uid());

DROP POLICY IF EXISTS "Goal owners can update shares" ON public.goal_shares;
CREATE POLICY "Goal owners can update shares"
    ON public.goal_shares FOR UPDATE
    USING (owner_id = auth.uid());

DROP POLICY IF EXISTS "Goal owners can delete shares" ON public.goal_shares;
...
DROP POLICY IF EXISTS "Team creators can create goals" ON public.team_goals;
CREATE POLICY "Team creators can create goals"
    ON public.team_goals FOR INSERT
    WITH CHECK (creator_id = auth.uid());

DROP POLICY IF EXISTS "Team admins can update goals" ON public.team_goals;
...
DROP POLICY IF EXISTS "Team creators can delete goals" ON public.team_goals;
...
-- RLS Policies for comments (updated to include team goals)
DROP POLICY IF EXISTS "Users can view comments on their goals/tasks" ON public.comments;
CREATE POLICY "Users can view comments on their goals/tasks"
    ...
    USING (
        ...
        (goal_id IS NOT NULL AND EXISTS (
            SELECT 1 FROM public.goal_shares WHERE goal_id = comments.goal_id AND shared_with_user_id = auth.uid()
        )) OR
        (task_id IS NOT NULL AND EXISTS (
            SELECT 1 FROM public.tasks WHERE id = task_id AND user_id = auth.uid()
        )) OR
        (team_goal_id IS NOT NULL AND EXISTS (
            SELECT 1 FROM public.team_members WHERE team_goal_id = comments.team_goal_id AND user_id = auth.uid()
        ))
    );
...
DROP POLICY IF EXISTS "Users can view public activity" ON public.activity_feed;
...
DROP POLICY IF EXISTS "Users can update own activity" ON public.activity_feed;
...
-- ============================================
-- PART 4: ENHANCE EXISTING GOALS TABLE
-- ============================================

-- Add visibility field to existing goals table
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'goals_visibility_check' 
        AND table_name = 'goals'
    ) THEN
        ALTER TABLE public.goals ADD CONSTRAINT goals_visibility_check CHECK (visibility IN ('public', 'private', 'followers'));
    END IF;
END $$;
...
-- Update existing goals RLS to include shared goals
...
CREATE OR REPLACE FUNCTION public.create_activity_entry(
    ...
    p_team_goal_id UUID DEFAULT NULL,
...
BEGIN
    INSERT INTO public.activity_feed (
        user_id, activity_type, goal_id, task_id,
        team_goal_id, metadata, is_public
    )
    VALUES (
        p_user_id, p_activity_type, p_goal_id, p_task_id,
        p_team_goal_id, p_metadata, p_is_public
    )
