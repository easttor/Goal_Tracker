...
-- Add comments
COMMENT ON TABLE habit_templates IS 'Pre-built habit templates for quick habit creation';
COMMENT ON TABLE habit_achievements IS 'User achievements and badges for habit milestones';
COMMENT ON TABLE habit_notes IS 'Daily notes and mood tracking for habit completions';
COMMENT ON TABLE habit_progress_photos IS 'Progress photos for visual habit tracking';
COMMENT ON COLUMN habits.category IS 'Habit category for organization and filtering';
COMMENT ON COLUMN habits.color IS 'Color theme for visual categorization';
COMMENT ON COLUMN habits.reminder_time IS 'Time for daily habit reminders';
COMMENT ON COLUMN habits.difficulty_level IS 'Habit difficulty level affecting achievement requirements';
